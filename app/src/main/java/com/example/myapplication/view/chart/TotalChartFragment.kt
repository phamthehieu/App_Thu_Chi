...
class TotalChartFragment : Fragment() {

    private lateinit var binding: FragmentTotalChartBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentTotalChartBinding.inflate(inflater, container, false)
    ...
    private fun setupPieChart() {
        ...
        val totalIncome = groupedData?.filter { it.incomeExpense.type == "Income" }
            ?.sumOf { it.incomeExpense.amount.replace(",", ".").toDouble() } ?: 0.0
        val totalExpense = groupedData?.filter { it.incomeExpense.type == "Expense" }
            ?.sumOf { it.incomeExpense.amount.replace(",", ".").toDouble() } ?: 0.0
